...
function session(el, r, ctx) {
  ...
  sess = { queue: buildQueue(D, r), done: 0, total: 0, stats: { again: 0, hard: 0, good: 0, easy: 0 }, shown: false, back: home + "/cards", home };
  sess.total = sess.queue.length;
  if (!sess.total) {
    el.innerHTML = '<div class="empty"><b>Nothing to study here</b>No cards are due in this topic. You can study ahead from the topic page.<p style="margin-top:14px"><a class="btn" href="' + sess.back + '">Back to topic</a></p></div>';
    return;
  }
  draw(el, ctx);
  cleanup();
